...
6. **frontAgain** Given a string, return true if the first 2 chars in the string also appear 
    at the end of the string, such as with "edited".
		...
		frontAgain("ed") → true
*/
				 	function frontAgain(sameTwo) {
						 if(sameTwo.substr(0, 2) === sameTwo.substr(-2)) {
							 return true;
						 } else {
							 return false;
						 }
...
10. **filterBigNumbers** Write a function that filters numbers greater than or equal to 100.
	...
	filterBigNumbers([]) → []
*/
						function filterBigNumbers(bigArray) {
							let bigNumber = [];
							for(let i = 0; i < bigArray.length; i++) {
								if(bigArray[i] >= 100) {
									bigNumber.push(bigArray[i]);
								}
							}
							return bigNumber;
						}
...
*/
							function filterMultiplesOfX(x, y) {
								let numbers = [3, 5, 1, 10, 20, 9, 18, 21, 42, 67];
								let newArray = [];
